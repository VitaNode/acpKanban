...
class SessionWebSocketService {
  ...
  Future<bool> connect(String cardId, {int retryCount = 0}) async {
    AppLogger.info(
        'Connect attempt for card $cardId (retry=$retryCount), currentCardId=$_currentCardId, isConnected=$_isConnected, hasLoadedHistory=$_hasLoadedHistory');

    if (_currentCardId != null && _currentCardId != cardId) {
      AppLogger.info(
          'Switching from card $_currentCardId to $cardId, disconnecting old connection');
      await disconnect();
    }

    if ((_channel != null || (_useProxy && _isConnected)) &&
        _currentCardId == cardId &&
        _isConnected) {
      AppLogger.info(
          'Reusing existing connection for card $cardId, clearing cache and requesting history');
      _clearCache();
        ...
        if (response.containsKey('result')) {
          ...
          _acpSub = _acpClient.messages.listen((msgStr) {
            try {
              ...
              if (msg['method'] == 'session/ws_event' &&
                  msg['params']?['card_id'] == _currentCardId) {
          ...
          _errorController
              .add(ErrorCopy.mapError(err['error_code'], err['message']));
    ...
    try {
      final config = await ConnectionConfigManager.getInstance()
          .then((m) => m.loadConfig());
      ...
      _reconnectCount = 0;
      _startHeartbeat();
      _channel!.stream.listen((data) => _handleMessage(data), onError: (e) {
        _isConnected = false;
      ...
      if (canMerge) {
        ...
        nextMeta.forEach((key, value) {
          if (key == 'thought') {
            ...
            final List<dynamic> currentTC =
                List.from(mergedMeta['tool_calls'] ?? []);
            final List<dynamic> nextTC = List.from(value as List? ?? []);
            for (var tc in nextTC) {
              final id = tc['tool_id'];
              final existingIdx =
                  currentTC.indexWhere((e) => e['tool_id'] == id && id != null);
              if (existingIdx != -1)
                currentTC[existingIdx] = tc;
              else
                currentTC.add(tc);
            }
  ...
  void _handleMessage(dynamic data) {
    try {
      ...
      switch (m['type']) {
        ...
        case 'agent_message_chunk':
          ...
          if (chunk.isNotEmpty) {
            if (_currentMessages.isNotEmpty &&
              ...
              _currentMessages[_currentMessages.length - 1] =
        ...
        case 'agent_thought_chunk':
          ...
          if (thought.isNotEmpty) {
            if (_currentMessages.isNotEmpty &&
              ...
              _currentMessages[_currentMessages.length - 1] =
                  last.copyWith(metadata: metadata);
            } else {
              _currentMessages.add(CardMessage(
        ...
        case 'error':
          _initializingController.add(false);
          _errorController.add(ErrorCopy.mapError(
              m['error_code'], m['message'] ?? 'Unknown agent error'));
          break;
      }
    } catch (e) {
      _initializingController.add(false);
  ...
  void _deliverAgUiEvent(AgUiEvent event) {
    ...
    if (event.toolCalls != null) {
      agEventMap['tool_calls'] = event.toolCalls!
          .map((tc) => {
                'tool_id': tc.toolId,
                'name': tc.name,
                'status': tc.status,
                'args': tc.args,
                'result': tc.result,
                if (tc.commandPreview != null)
                  'command_preview': tc.commandPreview,
    ...
    if ((event.eventType == 'agent_message_chunk' ||
      ...
      if (_currentMessages.isNotEmpty &&
        ...
        final last = _currentMessages.last;
        _currentMessages[_currentMessages.length - 1] =
            last.copyWith(content: last.content + chunk, seqId: event.seqId);
      } else {
        _currentMessages.add(CardMessage(
      ...
      _messageController.add(List.from(_currentMessages));
    } else if ((event.eventType == 'agent_thought_chunk' ||
            event.eventType == 'reasoning_message') &&
        (event.text != null || event.reasoning != null)) {
      ...
      if (_currentMessages.isNotEmpty &&
        ...
        final last = _currentMessages.last;
        _currentMessages[_currentMessages.length - 1] =
            last.copyWith(content: last.content + thought, seqId: event.seqId);
      } else {
        _currentMessages.add(CardMessage(
      ...
      final toolCall =
          event.toolCalls?.isNotEmpty == true ? event.toolCalls!.first : null;
      ...
      int existingIndex = _currentMessages.indexWhere(
          (m) => m.role == 'tool' && m.metadata?['tool_id'] == toolId);
      if (existingIndex != -1) {
        final existing = _currentMessages[existingIndex];
        final updatedMetadata =
            Map<String, dynamic>.from(existing.metadata ?? {});
        if ((updatedMetadata['name'] == null ||
                updatedMetadata['name'] == 'Unknown') &&
            toolName != 'Unknown') updatedMetadata['name'] = toolName;
        updatedMetadata['status'] = toolStatus;
        if (toolCall.args != null && toolCall.args!.isNotEmpty)
          updatedMetadata['arguments'] = toolCall.args;
        if (toolCall.commandPreview != null)
          updatedMetadata['command_preview'] = toolCall.commandPreview;
        if (toolCall.fileTargets != null)
          updatedMetadata['file_targets'] = toolCall.fileTargets;
        if (toolCall.opKind != null)
          updatedMetadata['op_kind'] = toolCall.opKind;
        ...
        final meta = <String, dynamic>{
          ...
          'tool_id': toolId,
          if (toolCall.commandPreview != null)
            'command_preview': toolCall.commandPreview,
          if (toolCall.fileTargets != null)
            'file_targets': toolCall.fileTargets,
      ...
      _messageController.add(List.from(_currentMessages));
    } else if (event.eventType == 'message_bundled' ||
        event.eventType == 'message_chunk') {
      ...
      if (event.toolCalls != null && event.toolCalls!.isNotEmpty) {
        metadata['tool_calls'] = event.toolCalls!
            .map((tc) => {
                  'tool_id': tc.toolId,
                  'name': tc.name,
                  'status': tc.status,
      ...
      try {
        ...
        if (_currentMessages.isNotEmpty &&
          ...
          _currentMessages[_currentMessages.length - 1] =
              agUiMessage.copyWith(metadata: {'type': 'plan_update'});
        } else {
          _currentMessages
              .add(agUiMessage.copyWith(metadata: {'type': 'plan_update'}));
        }
        _messageController.add(List.from(_currentMessages));
      } catch (e) {
      ...
      try {
        ...
        if (options != null)
          _configController.add(options
              .map((o) => ConfigOption.fromJson(o as Map<String, dynamic>))
              .toList());
      } catch (e) {
      ...
      try {
        ...
        if (commands != null)
          _commandController
              .add(commands.map((c) => c as Map<String, dynamic>).toList());
      } catch (e) {
      ...
      if (_currentMessages.isNotEmpty &&
          _currentMessages.last.role == 'assistant') {
        _currentMessages[_currentMessages.length - 1] =
            _currentMessages.last.copyWith(isComplete: true);
        _messageController.add(List.from(_currentMessages));
      }
    } else {
  ...
  void _tryDeliverBufferedEvents() {
    ...
    if (deliverCount == 0 && _eventBuffer.isNotEmpty) {
      ...
      AppLogger.warning(
          "Buffer gap detected. Forcing delivery of seqId=$oldestSeqId");
  ...
  Future<void> sendResponse(String id, Map<String, dynamic> result) async {
    if (_isConnected)
      await _send({'type': 'rpc_response', 'id': id, 'result': result});
  }
  ...
  Future<void> disconnect() async {
    ...
    if (_useProxy && _currentCardId != null && _isConnected) {
      try {
        await _acpClient.sendRequest('session/ws_proxy',
            {'action': 'disconnect', 'card_id': _currentCardId});
      } catch (e) {
  ...
  void _updateCardAndConfig(Map<String, dynamic> m) {
    if (m.containsKey('acp_session_id') ||
        m.containsKey('acp_provider_id') ||
        m.containsKey('input_tokens')) {
      _cardUpdateController.add(KanbanCard(
        id: _currentCardId ?? '',
        title: '',
        description: '',
        ...
        availableCommands:
            (m['available_commands'] as List?)?.cast<Map<String, dynamic>>(),
        inputTokens: m['input_tokens'] ?? 0,
        outputTokens: m['output_tokens'] ?? 0,
      ));
    }
