...
class SessionWebSocketService {
  ...
  Future<bool> connect(String cardId, {int retryCount = 0}) async {
    if (_channel != null && _currentCardId == cardId && _isConnected) return true;
    if (_currentCardId != cardId) {
      _messageController.add([]); _planController.add(null); _configController.add([]); _commandController.add([]);
      _reconnectCount = 0;
    }
    await disconnect(); _currentCardId = cardId;
    try {
      final uri = Uri.parse('${_baseUrl.replaceFirst('http', 'ws')}/ws/session/$cardId');
      _channel = WebSocketChannel.connect(uri);
      await _channel!.ready;
      _isConnected = true; _reconnectCount = 0; _startHeartbeat();
      _channel!.stream.listen((data) => _handleMessage(data as String), 
        onError: (e) { _isConnected = false; _reconnectIfNecessary(); },
        onDone: () { _isConnected = false; _reconnectIfNecessary(); });
      await _requestHistory();
      return true;
    } catch (e) {
      _isConnected = false;
      if (retryCount < 3) { await Future.delayed(Duration(seconds: 2)); return connect(cardId, retryCount: retryCount + 1); }
      return false;
    }
  }
  ...
  void _reconnectIfNecessary() {
    ...
    if (_currentCardId != null && _reconnectCount < 5) {
      _reconnectCount++;
      _reconnectTimer = Timer(Duration(seconds: 5 * _reconnectCount), () { if (!_isConnected && _currentCardId != null) connect(_currentCardId!); });
    }
  }
  ...
  void _startHeartbeat() {
    ...
    _heartbeatTimer = Timer.periodic(const Duration(seconds: 30), (t) {
      if (_isConnected && _channel != null) { try { _channel!.sink.add(jsonEncode({'type': 'ping'})); } catch (e) { _isConnected = false; } }
    });
  }

  Future<void> _requestHistory() async { if (_channel != null && _isConnected) _channel?.sink.add(jsonEncode({'type': 'get_history'})); }
  ...
  void _handleMessage(String data) {
    try {
      ...
      switch (m['type']) {
        ...
        case 'config_options': _configController.add((m['options'] as List?)?.map((x) => ConfigOption.fromJson(x)).toList() ?? []); break;
        case 'available_commands': _commandController.add((m['commands'] as List?)?.cast<Map<String, dynamic>>() ?? []); break;
        case 'message_added':
        case 'refresh': _requestHistory(); break;
      }
    } catch (e) { print('WS Parse Error: $e'); }
  }

  Future<void> setConfigOption(String name, String value) async {
    if (_channel != null && _isConnected) _channel!.sink.add(jsonEncode({'type': 'set_config_option', 'name': name, 'value': value}));
  }
